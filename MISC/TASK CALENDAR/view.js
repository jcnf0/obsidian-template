...
if (dailyNoteFormat) { if (dailyNoteFormat.match(/[|\\YMDWwd.,-_: \[\]]/g).length != dailyNoteFormat.length) { dv.span('> [!ERROR] The `dailyNoteFormat` contains invalid characters'); return false }};
...
function getMonth(tasks, month) {
	...
	for (w=1; w<7; w++) {
		...
		for (i=starts;i<starts+7;i++) {
			...
			if (!dailyNoteFolder) {var dailyNotePath = moment(currentDate).format("YYYY-MM-DD_ddd")} else {var dailyNotePath = dailyNoteFolder+"/"+moment(currentDate).format("YYYY-MM-DD_ddd")};
